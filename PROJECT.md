...
**Gyeongbokgung is the starting focus.** The supplied broader context brief discusses Donggwol as a long-term interest; the current conversation retains Gyeongbokgung as the practical first project. Gyotaejeon now has linked descriptive and archaeological sources, but no building geometry extracted; it is not a required first model.
...
**Modeling requirement, clarified 2026-09-22:** every commissioned building must become a complete, sensibly assembled Korean palace timber building appropriate to its type. Where building-specific evidence is missing, use explicit, revisable assumptions and suitable fallback construction. A highly accurate collection of parts with necessary components absent is a failed building outcome. This requirement applies throughout modeling; research uncertainty remains recorded without becoming a reason to omit required geometry or reduce the commissioned scope.

The immediate research priorities are **Sajeongjeon's dimensional attributes first**, alongside a layered, correctable body of knowledge for distinct palace building types. The owner's “platonic ideal” now means several working configurations—halls, residences, covered walkways and enclosed/mixed ranges—sharing a vocabulary while preserving their different arrangements. The [living briefs](typologies/README.md) support modeling assumptions and will be refined over the coming weeks as evidence accumulates. Their relationships and alternatives should grow from inspected evidence and useful comparisons. Practical defaults can be adopted in modeling without being presented as measured historical facts. Broader modeling-workflow design is reserved for a separate discussion; [working ideas](brainstorming/complete-halls-and-fallbacks.md) are deliberately non-canonical.
